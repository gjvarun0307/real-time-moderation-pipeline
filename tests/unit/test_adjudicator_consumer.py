...
from adjudicator import consumer as consumer_module
...
class FakeAIOKafkaConsumer:
    def __init__(self, *_topics, **kwargs) -> None:
        self.started = False
        self.stopped = False
        self.commits = 0
...
@pytest.fixture
async def consumer(monkeypatch) -> tuple[EscalateConsumer, FakeAIOKafkaConsumer]:
    monkeypatch.setattr(consumer_module, "AIOKafkaConsumer", FakeAIOKafkaConsumer)
    c = EscalateConsumer(
        bootstrap_servers="fake:9092",
        topic="moderation.escalate",
        group_id="adjudicator",
        max_poll_records=5,
    )
    await c.start()
    fake = c._consumer
...
async def test_start_passes_max_poll_records(consumer):
    _c, fake = consumer
    assert fake.kwargs["max_poll_records"] == 5
...
async def test_current_lag_sums_across_assigned_partitions(consumer):
    c, fake = consumer
    fake._assignment = {"moderation.escalate-0", "moderation.escalate-1"}
    fake._highwater = {"moderation.escalate-0": 100, "moderation.escalate-1": 50}
    fake._position = {"moderation.escalate-0": 90, "moderation.escalate-1": 50}
